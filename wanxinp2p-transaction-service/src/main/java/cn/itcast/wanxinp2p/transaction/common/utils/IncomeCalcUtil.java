...
public class IncomeCalcUtil {
    ...
    public static BigDecimal getIncomeTotalInterest(BigDecimal invest, BigDecimal yearRate, int month) {
        ...
        for (int i = 1; i < month + 1; i++) {
            ...
            BigDecimal sub = BigDecimal.valueOf(Math.pow(1 + monthRate, month)).subtract(BigDecimal.valueOf(Math.pow(1 + monthRate, i - 1)));
            BigDecimal monthInterest = multiply.multiply(sub).divide(BigDecimal.valueOf(Math.pow(1 + monthRate, month) - 1), 6, RoundingMode.DOWN);
